...
void t4()
{
  ...
  TFile* outFile = new TFile("rootfile/t4.root","recreate");
  TH2D* hist = new TH2D("hist", ";Ch;ZAP#", 260,0,260, 31,1,32);
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      asad[id][ageti*64+chi2][55-runNum] = adcMax-ped/100.;
	      peak[id][ageti*64+chi2][55-runNum] = adcMax-ped/100.;
	      peakid[id][ageti*64+chi2][55-runNum] = padid;
/*
