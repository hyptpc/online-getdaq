...
void t7()
{
  ...
  TFile* outFile = new TFile("rootfile/t7.root","recreate");
  ...
  int zapNum[N_ASAD] = {9, 10, 11, 12, 13, 14, 15, 16};
  ...
  for(int i=0; i<N_ASAD; i++)
	...
	poly->SetBinContent(peakid[i][j][N_VOL-1], 5);
      ...
      else if( peak[i][j][N_VOL-1] <300 &&  peak[i][j][N_VOL-1]>50 ) {
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMin>fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-88] = adcMin-ped/100.;
	      asad[id][ageti*64+chi2][runNum-88] = fabs(adcMin-ped/100.);
	      peak[id][ageti*64+chi2][runNum-88] = fabs(adcMin-ped/100.);
	      peakid[id][ageti*64+chi2][runNum-88] = padid;
	      /*
