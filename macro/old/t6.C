...
void t6()
{
  ...
  TFile* outFile = new TFile("rootfile/t6.root","recreate");
  ...
  int zapNum[N_ASAD] = {27, 26, 10, 13, 29, 0, 8, 18};
  int asadNum[N_ASAD] = {1, 2, 13, 9, 6, 5, 7, 3};
...
//#if 1
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMin>fadc[ageti][chi][tbi])
		{
		  adcMin=fadc[ageti][chi][tbi];
		  tbmin=tbi;
		}
	      }
	      //peak[id][ageti*64+chi2][runNum-80] = adcMin-ped/100.;
	      asad[id][ageti*64+chi2][runNum-80] = fabs(adcMin-ped/100.);
	      peak[id][ageti*64+chi2][runNum-80] = fabs(adcMin-ped/100.);
	      peakid[id][ageti*64+chi2][runNum-80] = padid;
	      /*
