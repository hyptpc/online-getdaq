...
/*
  12 voltages : 0.5, 1.0, ... , 6.0 V 
  8 AsAd, ZAP boards, 256 ch each
  peak[8][256][12]
*/
#define N_ASAD 8
#define N_CH   256
...
void t1()
{
  ...
  TFile* outFile = new TFile("rootfile/t1.root","recreate");
  ...
  int zapNum[N_ASAD] = { 13, 14, 12, 15, 16, 11, 10, 9};
//  int zapNum_cor[N_ASAD];
//  int asadNum_cor[N_ASAD];
//  for(int i=0; i<N_ASAD; i++) {
//    if(i%2==0) {
//      zapNum_cor[i] = zapNum[i+1];
//      asadNum_cor[i] = asadNum[i+1];
//    }
//    else if(i%2==1){
//      zapNum_cor[i] = zapNum[i-1];
//      asadNum_cor[i] = asadNum[i-1];
...
//  }
  ...
  for(int i=0; i<N_ASAD; i++)
	...
	poly->SetBinContent(peakid[i][j][N_VOL-1], 1);
      ...
      /*
      if( peak[i][j][N_VOL-1] == -9999 )
	hist->Fill(j, zapNum_cor[i], 4);
      else if( peak[i][j][N_VOL-1] <700 )
	hist->Fill(j, zapNum_cor[i], 2);
      else if( peak[i][j][N_VOL-1] > 1700) 
	hist->Fill(j, zapNum_cor[i], 3);
      else 
	hist->Fill(j, zapNum_cor[i], 1);
	*/

      if(asad[i][j][N_VOL-1]!=-9999) 
	asad_hist->Fill(j, i, asad[i][j][N_VOL-1]);
    }
  ...
  c1->Divide(4,2);
  for(int i=0; i<N_ASAD; i++) {
    //c1[i] = new TCanvas(Form("c1_%d",i),Form("AsAd%d",i),800,600);
    //c1[i]->cd();
    c1->cd(i+1)->SetGrid();
    c1->cd(i+1)->SetMargin(0.15, 0.1, 0.1, 0.1);
    for(int j=0; j<N_CH; j++) {
      if(j==0) {
	graph[i][j]->SetTitle(Form("AsAd\#%d + ZAP\#%d; Input [V]; ADC Max [ch]", asadNum[i], zapNum[i]));
	graph[i][j]->SetMinimum(0);
	graph[i][j]->SetMaximum(2500);
	graph[i][j]->Draw("APL");
      }
      else graph[i][j]->Draw("PL");
    }
  }
...
void getPeak(int runNum, int eventNum = 1)
{
  ...
  for(int coboNum = 0; coboNum<COBONUM ;coboNum++)
  {
    ...
    datafile.str("");
      datafile<<"/data"<<coboNum<<"/zaptest/cobo"<<coboNum<<"/run_"<<std::setfill('0')<<std::setw(4)<<runNum<<".dat";
...
//#if 1
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
	    ...
	    {
	      if( asadId == 0 ) id = coboNum*4+1;
	      else if( asadId == 1 ) id = coboNum*4+0;
	      else if( asadId == 2 ) id = coboNum*4+3;
	      else if( asadId == 3 ) id = coboNum*4+2;
	      else { cout << "asadId error " << endl; exit(-1); }
	      //padid=channelmap[ageti][coboNum*4+asadId][chi2];
	      padid=channelmap[ageti][id][chi2];
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi][runNum-11] = adcMax-ped/100.;
	      asad[id][ageti*64+chi2][runNum-11] = adcMax-ped/100.;
	      //peak[coboNum*4+asadId][ageti*64+chi2][runNum-11] = adcMax-ped/100.;
	      //peakid[coboNum*4+asadId][ageti*64+chi2][runNum-11] = padid;
	      peak[id][ageti*64+chi2][runNum-11] = adcMax-ped/100.;
	      peakid[id][ageti*64+chi2][runNum-11] = padid;
/*
