...
void t5()
{
  ...
  TFile* outFile = new TFile("rootfile/t5.root","recreate");
  ...
  int zapNum[N_ASAD] = {27, 26, 10, 13, 29, 16, 8, 18};
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMin>fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-63] = adcMin-ped/100.;
	      asad[id][ageti*64+chi2][runNum-63] = adcMin-ped/100.;
	      peak[coboNum*4+asadId][ageti*64+chi2][runNum-63] = adcMin-ped/100.;
/*
