...
void gain(int runNum)
{
  ...
  graph = new TGraph();
  hist = new TH1D("hist","Peak height", 4000,0,4000);
  //while(!UserStop)
  ...
  for( int eventi=0; eventi<eventNum; eventi++)
  {
	...
	if( (int)get_header.m_FrameType[1]==2 ) // full
	{
	  ...
	  for(int ageti = 0; ageti<4; ageti++)
	  {
		...
		cout << "padid : " << padid <<",  peak : " << adcMax - (double)adcSum/ntb  << endl;
		getchar();
