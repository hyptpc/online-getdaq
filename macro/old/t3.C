...
void t3()
{
  ...
  TFile* outFile = new TFile("rootfile/t3.root","recreate");
  ...
  int zapNum[N_ASAD] = { 25, 26, 27, 28, 29, 30, 8};
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-38] = adcMax-ped/100.;
	      asad[id][ageti*64+chi2][runNum-38] = adcMax-ped/100.;
	      peak[id][ageti*64+chi2][runNum-38] = adcMax-ped/100.;
	      peakid[id][ageti*64+chi2][runNum-38] = padid;
/*
