...
void t2()
{
  ...
  TFile* outFile = new TFile("rootfile/t2.root","recreate");
  ...
  int zapNum[N_ASAD];
  for(int i=0; i<N_ASAD; i++)  zapNum[i] = 17+i;
  ...
  */
  ...
  for(int i=0; i<N_VOL; i++) {
    if(i<8) voltage[i] = 0.5 + 0.5*i;
    else voltage[i] = voltage[7] + 1*(i-7);
    runNum[i]   = 27+i;
  }
  // CH map ===================================================
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-27] = adcMax-ped/100.;
	      asad[id][ageti*64+chi2][runNum-27] = adcMax-ped/100.;
	      peak[id][ageti*64+chi2][runNum-27] = adcMax-ped/100.;
	      peakid[id][ageti*64+chi2][runNum-27] = padid;
/*
