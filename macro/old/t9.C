...
void t9()
{
  ...
  TFile* outFile = new TFile("rootfile/t9.root","recreate");
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-run_init] = adcMax-ped/100.;
	      asad[id][ageti*64+chi2][runNum-run_init] = fabs(adcMax-ped/100.);
	      peak[coboNum*4+asadId][ageti*64+chi2][runNum-run_init] = fabs(adcMax-ped/100.);
	      /*
