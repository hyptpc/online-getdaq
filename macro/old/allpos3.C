...
void allpos3()
{
  ...
  TFile* outFile = new TFile("rootfile/allpos3.root","recreate");
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMin>fadc[ageti][chi][tbi])
		{
	      ...
	      peak[coboNum*4+asadId][ageti*64+chi2][runNum-run_init] = ped/100.-adcMin;
	      //cout << "ped: " << ped/100. <<", min: " << adcMin << ", peak : " << ped/100.-adcMin << endl;
	      //getchar();
