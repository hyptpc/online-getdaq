...
void t10()
{
  ...
  TFile* outFile = new TFile("rootfile/t10.root","recreate");
  ...
  int zapNum[N_ASAD] = {17, 18, 19, 20, 21, 22, 23, 24};
  int zapNum_cor[N_ASAD];
  for(int i=0; i<N_ASAD; i++) {
    if(i%2==0) zapNum_cor[i] = zapNum[i+1];
    else if(i%2==1) zapNum_cor[i] = zapNum[i-1];
  }
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMin>fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-run_init] = adcMin-ped/100.;
	      asad[id][ageti*64+chi2][runNum-run_init] = fabs(adcMin-ped/100.);
	      peak[coboNum*4+asadId][ageti*64+chi2][runNum-run_init] = fabs(adcMin-ped/100.);
	      /*
