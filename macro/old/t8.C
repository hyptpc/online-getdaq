...
void t8()
{
  ...
  TFile* outFile = new TFile("rootfile/t8.root","recreate");
	...
	for(int ageti = 0; ageti<4; ageti++)
	{
	  for(int chi = 0; chi<68; chi++)
	  {
		...
		if(tbi >TBCUTMIN && tbi<TBCUTMAX &&adcMax<fadc[ageti][chi][tbi])
		{
	      ...
	      //peak[id][ageti*64+chi2][runNum-94] = adcMax-ped/100.;
	      asad[id][ageti*64+chi2][runNum-94] = fabs(adcMax-ped/100.);
	      peak[id][ageti*64+chi2][runNum-94] = fabs(adcMax-ped/100.);
	      peakid[id][ageti*64+chi2][runNum-94] = padid;
	      /*
