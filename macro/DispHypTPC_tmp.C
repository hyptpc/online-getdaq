	...
	for( Int_t ageti=0; ageti<4; ageti++ ){
	  for(Int_t chi = 0; chi<NumOfChannelAGET; chi++){
		...
		if( adcMax<fadc[ageti][chi][tbi] ){
	      ...
	      if(adcMax-((Double_t)adcSum/ntb)>50. ){
		h1->SetBinContent( h1->FindBin(getRowID(padid), getLayerID(padid)), adcMax-((Double_t)adcSum/ntb) );
		TVector3 position = getPoint( padid );
		h2->SetBinContent( h2->FindBin(position.z(), position.x()), adcMax-((Double_t)adcSum/ntb) );
	      }
...
void current( void )
{
  ...
  std::ifstream frun("/home/axis/ganacq_manip/"+dir+"/test.run_number");
  if(!frun.is_open())
    {
      std::cerr << " /tmp/sync.log does not exist. " << std::endl;
      std::exit(-1);
    }
  frun >> g_run_number;
  // ------------
...
void DispHypTPC_tmp( void )
{
  ...
  if( h1 ) delete h1;
  h1 = new TH2D( "h1",";row;layer", 80,0,80,32,0,32 );
  if( h2 ) delete h2;
  h2 = new TH2D( "h2","Z;X", 100,-300,300,100,-300,300 );
